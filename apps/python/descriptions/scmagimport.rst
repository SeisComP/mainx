...
:term:`SCML` :ref:`api-python-datamodel-event` file into |scname|.
...
#. Use :ref:`scevent` to associate the origin to an SeisComP event. In this step
   the foreign origin is send via HTTP POST to the :ref:`scevent` REST API
   `/api/1/try-to-associate` which responds with an event ID if the association
   was successful.
#. Retrieve the event and the preferred origin from the local |scname| system.
